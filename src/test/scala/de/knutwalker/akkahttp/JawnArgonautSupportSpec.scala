...
object JawnArgonautSupportSpec extends Specification with JawnArgonautSupport with FutureMatchers with AfterAll {

  case class Foo(bar: String, baz: Int, qux: List[Boolean])
  object Foo {
    implicit val codecJsonFoo: CodecJson[Foo] =
      CodecJson.casecodec3(Foo.apply, Foo.unapply)("bar", "baz", "qux")
  }
  ...
  title("Specification for Jawn-Argonaut Support")

  "JawnArgonautSupportSpec" should {
    import system.dispatcher
    implicit val eenv = ExecutionEnv.fromExecutionContext(system.dispatcher)
    "enable marshalling of an A for which an EncodeJson[A] exists" in {
      "The marshalled entity" should {
        ...
        "have UTF-8 as charset" >> {
          entity.contentType.charset() must be(`UTF-8`)
        }
      ...
      "The marshalled entity with overridden PrettyParams" should {
        implicit val makeItPretty: PrettyParams = Argonaut.spaces2
        val entity = Await.result(Marshal(foo).to[RequestEntity], 100.millis)
    ...
    "enable unmarshalling of an A for which a DecodeJson[A] exists" in {
  ...
  def mkEntity(s: String, strict: Boolean = false) = {
    if (strict) {
      ...
      val source = Source(() ⇒ s.grouped(8).map(ByteString(_)))
